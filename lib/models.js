...
function Modelator(config = {}){
  const modelator = this;
  modelator.id = config.id || 'noid';
  ...
  let keys;
  Object.defineProperty(modelator, "keys",{
    enumerable : true,
  ...
  modelator.keys = config.keys;
}
  Modelator.prototype.configure = function(config = {}){
    for(const key in config){
      this[key] = config[key];
    }
    return this;
  };
  Modelator.prototype.get = function(data, input, cb, uid, keys){
    this.action('get', data, input, cb, uid, keys);
  };
  Modelator.prototype.insert = function(data, input, cb, uid, keys){
    this.action('insert', data, input, cb, uid, keys);
  };
  Modelator.prototype.update = function(data, input, cb, uid, keys){
    this.action('update', data, input, cb, uid, keys);
  };
  Modelator.prototype.remove = function(data, input, cb, uid, keys){
    this.action('remove', data, input, cb, uid, keys);
  };
  Modelator.prototype.action = function(action, data, input, cb, uid, keys){
    const modelator = this;
    ...
    Schema.prototype.controller.call(modelator, subcontext, function(error){
      if(error){
...
function Field(config = {}){
  ...
  Field.prototype.configure = Modelator.prototype.configure;
...
module.exports.SchemaArray = SchemaArray;
util.inherits(SchemaArray, Schema);
function SchemaArray(config = {}){
