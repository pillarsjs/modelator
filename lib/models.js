...
const Controllable = module.exports.Controllable = function(config = {}){
  ...
  Controllable.prototype.fire = function(context, cb){
    ...
    const fromData = ["projection", "get"].indexOf(context.action) >= 0; // ["insert", "update", "remove"]
    ...
    console.log((" " + context.action.charAt(0).toUpperCase() + context.action.slice(1) + " ").bgMagenta + (" " + (context.path.join(".") || '.')).magenta, traceResults(context.fieldData));
    ...
    if(every || context.result !== undefined){
      ...
      if(controller === Schema.prototype.controller){
        context.owner = (!context.uid || !context.fieldData || context.fieldData._owner === context.uid);
        context.guest = (context.fieldData && Array.isArray(context.fieldData._guests) && context.fieldData._guests.indexOf(context.uid) >= 0);
      }
      ...
      const keysCheck = controllable.internal? true : controllable.keys.check(context.action, context.keys.concat(ownkeys));
      console.log(" Cheking: ".bgYellow + (" " + (context.owner? '(owner) ' : '') + (context.guest? '(guest) ' : '') + (controllable.internal? '(internal) ' : '')).yellow + (keysCheck? 'ok!'.green : 'nop!'.red));
      if(keysCheck){
        controller = controller.bind(controllable);
        ...
        if(controllable.i18n){
          ...
          for(const lang of context.modelator.languages){
            ...
            const subcontext = Object.assign({}, context, {index : lang, fieldData, fieldInput, result: undefined, path, parent : context});
            for(const handler of handlers){              
              subcontext.chain = chain;
              chain.add(handler, context, fieldHandlerCallback.bind(subcontext));
              chain.add(fieldHandle, subcontext, cb);
            }
          }
          context.chain = chain;
          chain.add(controller, context, fieldHandlerCallback.bind(context));
          chain.add(fieldHandle, context, cb);
        } else {
          for(const handler of handlers){
            context.chain = chain;
...
function fieldHandle(context, cb, next){
  if(!context.parent){
    return cb();
  }
  // TODO: Handlers naming
  if(context.error){
    console.log(" Error: ".bgRed,     (context.path.join(".") || '.').bold.grey, context.error);
...
function arrayHandle(context, next){
  const resultPath = context.path.slice(0,-1).join('.');
  if(context.action === 'insert'){
    context.inserts[resultPath] = context.inserts[resultPath] || [];
    context.inserts[resultPath].push(context.result);
  } else if(context.action === 'update'){
    context.updates[resultPath] = context.updates[resultPath] || [];
    context.updates[resultPath].push(context.result);
  } else if(context.action === 'remove'){
    context.removes[resultPath] = context.removes[resultPath] || [];
    context.removes[resultPath].push(context.result);
  }
  next();
}
...
const Schema = module.exports.Schema = function(config = {}){
  ...
  Schema.prototype.controller = function(context, cb){
    ...
    // console.log("SCHEMA".black.bgWhite, context.path, context.result, indexes);
    ...
    for(const field of schema.schema){
      ...
      const subcontext = Object.assign({}, context, {index, fieldData, fieldInput, result: undefined, path, parent : context});
      const controller = field.fire.bind(field, subcontext);
      chain.add(controller);
    }
    for(const index of indexes){
      context.errors[context.path.concat([index]).join('.')] = 'unknow-input-field';
    }
...
// TODO: schema inheritance actions control (on insert no update, on remove no other action...)
const SchemaArray = module.exports.SchemaArray = function(config = {}){
  ...
  SchemaArray.prototype.controller = function(context, cb){ // cb === fieldHandlerCallback binded with context ===> function(error){ context = this}
    ...
    for(let index = 0; index < result.length; index++){
      ...
      chain.add(schemaArray.fire.bind(schemaArray, parent, Schema.prototype.controller));

      parent.end = cb;
      parent.chain = chain;
      chain.add(arrayHandle, parent);
      chain.add(fieldHandle, parent, cb);
...
const Modelator = module.exports.Modelator = function(config = {}){
  ...
  Modelator.prototype.do = function(action, data, input, cb, uid, keys){
    ...
    // TODO: add file store? do fs actions on allOk. (succes actions?)
    // TODO: review deep array extructures
    ...
    const removes = {};
    const path = [];
    const context = {modelator, action, data, input, uid, keys, fieldData, fieldInput, errors, inserts, updates, removes, path};

    modelator.fire(context, function(){
      if(Object.keys(context.errors).length > 0){
        cb(context.errors, context);
      } else {
        cb(null, context);
      }
    });
...
const myModelator = new Modelator({
  ...
  schema : [
    ...
    new SchemaArray({
      ...
      on : {
        insert : [function(context,cb){
          console.log("INSERT EVENT".bgGreen);
          cb();
        }]
      },
      schema: [
...
const resultShow = function(error, context){
  console.log("\n  MODELATOR\n".black.bgWhite,
    "\n    ERRORS:\n".bgRed, JSON.stringify(context.errors, undefined, 2),
    "\n    MAIN:\n".bgCyan, JSON.stringify(context.result, undefined, 2),
    "\n    INSERTS:\n".bgGreen, JSON.stringify(context.inserts, undefined, 2),
    "\n    UPDATES:\n".bgGreen, JSON.stringify(context.updates, undefined, 2),
    "\n    REMOVES:\n".bgGreen, JSON.stringify(context.removes, undefined, 2)
  );
};
