...
const TreeArray = function(array){
  ...
  //console.log("nuevoArray".bgRed, treeArray);
  //proto.constructor = TreeArray;
  var proto = Object.assign([], Array.prototype, Tree.prototype, {constructor: TreeArray});
  //console.log("PROTO".bgMagenta, proto);
...
const Controllable = module.exports.Controllable = function(config = {}){
    ...
    if(every || context.result !== undefined){
      ...
      if(keysCheck){
        ...
        for(const handler of handlers){
          chain.add(controllable.chainHandler, handler, context, controller, cb);
        }
        chain.add(function chainEnd(context, cb){
          console.log(" <- Result: ".bgCyan, (context.path.join(".") || '.').bold.grey, context.result);
          cb();
        },context, cb).pull();
      } else {
  ...
  Controllable.prototype.chainHandler = function(handler, context, controller, cb){
    // estoy aqui
    const chain = this;
...
const Field = module.exports.Field = function (config = {}){
  ...
  Field.prototype.controllerI18n = function(context, cb){
    ...
    context.errors = new TreeArray(context.errors);
    ...
    for(const lang of context.modelator.languages){
      ...
      chain.add(field.init, subcontext, controller);
    }
    chain.add(cb,context.errors).pull();
  };
  Field.prototype.init = function(context, controller){
    // es el que inicia el fire del field, es el iterador de una cadena de llamadas asincronas que se realiza para cada field.
    const chain = this;
    const chainCallback = function(error){
      // TODO: review 'this'
      console.log("pasando por".bgCyan,context.parent.path,context.index);
      if(error && error.length > 0){
        context.parent.errors[context.index] = error;
      } else {
        context.parent.result[context.index] = context.result;
      }
      chain.next();
    };
    controller(context, chainCallback);
  };
...
const Schema = module.exports.Schema = function(config = {}){
  ...
  Schema.prototype.controller = function(context, cb){
    ...
    const chain = new Chain();
    console.log("SCHEMA INI".black.bgWhite,schema.schema); // recibe duplicados los fields de schema.schema ??!!!
    for(const field of schema.schema){
      const index = field.id;
      console.log("FIELD".bgCyan, context.path, index);
      ...
      const controller = field.fire.bind(field);
      chain.add(schema.init, subcontext, controller);
    }
    chain.add(cb,context.errors).pull();
  };
...
const SchemaArray = module.exports.SchemaArray = function(config = {}){
  ...
  SchemaArray.prototype.controller = function(context, cb){
    ...
    // for(const index of context.result){
    for(let index = 0; index < context.result.length; index++){
      ...
      chain.add(schemaArray.init, subcontext, controller);
    }
    chain.add(cb,context.errors).pull();
  };
...
const Modelator = module.exports.Modelator = function(config = {}){
  ...
  Modelator.prototype.do = function(action, data, input, cb, uid, keys){
    ...
    }, Schema.prototype.controller); // no es necesario bindear a this, todas la llamadas a este metodo se realizan desde la instancia del objeto
  };
