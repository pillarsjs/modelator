...
const Field = module.exports.Field = function (config = {}){
  ...
  Field.prototype.controllerI18n = function(context, cb){
    ...
    for(const lang of context.modelator.languages){
      ...
      console.log("FieldResultHanler",field.resultHandler);
      chain.add(field.resultHandler, subcontext, controller, errors, result);
    }
  ...
  Field.prototype.resultHandler = function(subcontext, controller, errors, result){
    const chain = this;
    // console.log("result handler:",field);
    // controller = field.fire.bind(field);
    // controller = this.fire;
    // buscar diferencia en schemaArray
    const chainCallback = function(error){
      if(error){
...
const Schema = module.exports.Schema = function(config = {}){
  ...
  Schema.prototype.controller = function(context, cb){
    ...
    for(const field of schema.schema){
      ...
      chain.add(schema.resultHandler, subcontext, controller, errors, result);
    }
    chain.add(function chainEnd(){
      if(errors.length > 0){
        cb(errors);
  ...
  SchemaArray.prototype.controller = function(context, cb){
    ...
    for(const index in context.result){
      ...
      const controller = Schema.prototype.controller.bind(schemaArray);
       ...
       console.log("schemaArrayResultHanler",schemaArray.resultHandler);
      chain.add(schemaArray.resultHandler, subcontext, controller, errors, result);
    }
    chain.add(function chainEnd(){
...
const Modelator = module.exports.Modelator = function(config = {}){
  ...
  Modelator.prototype = Object.assign({resultHandler:Field.prototype.resultHandler},Controllable.prototype);
