...
const Chain = function(){
  ...
  Chain.prototype.pull = function(self){
    if(this.chainlinks.length > 0){
      this.chainlinks[0].func.apply(self,this.chainlinks[0].args.concat([this.next.bind(this)]));
      // mete en los parametros un ultimo parametro next, esto implica que al hacer cb() no llega a null el primero parametro (error)
    }
  };
...
const TreeArray = function(array){
  var treeArray = Array.apply(this,array);
  var proto = Object.assign([], Array.prototype, Tree.prototype, {constructor: TreeArray});
  Object.setPrototypeOf(treeArray, proto);
  return treeArray;
};
...
// Oder, field.fire -> handler chain [controller,...handler list] -> fire return.
    ...
    if(every || context.result !== undefined){
      ...
      if(keysCheck){
        ...
        if(controllable.i18n){
          ...
          context.errors = new Tree(context.errors);
          for(const lang of context.modelator.languages){
            ...
            const subcontext = Object.assign({i18nAvoid : true}, context, {index, fieldData, fieldInput, path, parent : context});
            for(const handler of handlers){
              chain.add(controllable.fireChain, handler, subcontext, cb);
            }
            chain.add(subFireEnd, subcontext);
          }
        } else {
          for(const handler of handlers){
            chain.add(controllable.fireChain, handler, context, cb);
          }
        }

        chain.add(function fireEnd(context, cb){
          console.log(" <- Result: ".bgCyan, (context.path.join(".") || '.').bold.grey, context.errors, context.result);
          cb();
        }, context, cb).pull();
...
function subFireEnd(context, next){
  console.log(" <- SubResult: ".bgCyan, (context.path.join(".") || '.').bold.grey, context.errors, context.result);
  next();
}

  Controllable.prototype.fireChain = function(handler, context, cb, next){
    //const chain = this;
    const chainCallback = function(error){
      ...
      const isError = typeof context.errors === 'object'? (Object.keys(context.errors) > 0) : context.errors;
      // TODO: Handlers naming
      if(isError){
        console.log(" <- Error: ".bgRed, context.errors, context.path, context.parent.path);
        context.parent.errors[context.index] = context.errors;
        cb();
      } else {
        console.log("    " + " Handling: ".bgCyan, context.result, context.path, context.parent.path);
        context.parent.result[context.index] = context.result;
        next();
      }
    };
    handler(context, chainCallback);
  };
...
const Schema = module.exports.Schema = function(config = {}){
  ...
  Schema.prototype.controller = function(context, cb){
    ...
    for(const field of schema.schema){
      ...
      const subcontext = Object.assign({}, context, {index, fieldData, fieldInput, path, parent : context});
      const controller = field.fire.bind(field, subcontext);
      chain.add(controller); // fire cb llama a chain.next de la chain superior. se pierde el error de array en el chain next(error)
    }
...
const SchemaArray = module.exports.SchemaArray = function(config = {}){
  ...
  SchemaArray.prototype.controller = function(context, cb){
    const schemaArray = this;
    //console.log("SCHEMA-ARRAY".black.bgWhite, context.path);
    ...
    const chain = new Chain();
    for(let index = 0; index < context.result.length; index++){ // for(const index in context.result){ // for(let index = 0; index < context.result.length; index++){
      console.log("INDEX".bgWhite, context.path, index);
      if(!Number.isInteger(index)){
        return cb('invalid-schema-array-index');
      }

      context.result[index] = new Tree();
      context.errors[index] = new Tree();

      const parent = Object.assign({}, context, {
        result : context.result[index],
        errors : context.errors[index],
        index,
      ...
      for(const field of schemaArray.schema){
        console.log("FIELD".bgWhite, parent.path, field.id);
        ...
        const subcontext = Object.assign({}, context, {index : field.id, fieldData, fieldInput, path, parent});
        console.log("FIELD DATA".bgWhite, parent.result,fieldInput);
        const controller = field.fire.bind(field, subcontext);
        chain.add(controller); // fire cb llama a chain.next de la chain superior. se pierde el error de array en el chain next(error)
      }
      chain.add(subFireEnd, parent);
    }
    chain.add(cb,undefined).pull();
    ...
    const chain = new Chain();
    for(let index = 0; index < context.result.length; index++){ // for(const index in context.result){ // for(let index = 0; index < context.result.length; index++){
      //console.log("INDEX".bgCyan, context.path, index);
      if(!Number.isInteger(index)){
        return cb('invalid-schema-array-index');
      }
      ...
      const subcontext = Object.assign({}, context,{index, fieldData, fieldInput, path, parent : context});
      const controller = schemaArray.fire.bind(schemaArray, subcontext);
...
const Modelator = module.exports.Modelator = function(config = {}){
  ...
  Modelator.prototype = Object.assign({init:Field.prototype.init},Controllable.prototype);
  Modelator.prototype.constructor = Modelator;
  Modelator.prototype.controller = Schema.prototype.controller;
  Modelator.prototype.do = function(action, data, input, cb, uid, keys){
    ...
    }); // no es necesario bindear a this, todas la llamadas a este metodo se realizan desde la instancia del objeto
  };
