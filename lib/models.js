  ...
  Controllable.prototype.fire = function(context, cb, controller){
    ...
    const chainHandler = function(handler){
      const chainCallback = function(error){
        console.log("Modeling:".yellow, context.path.join("/").cyan, context.result);
        if(error){
      ...
      // Parece un error con el fireresult del root.
    };
...
const Field = module.exports.Field = function (config = {}){
  ...
  field.on("get",field.get);
  field.on("insert",field.set);
  field.on("update",field.set);
  field.on("remove",field.set);
};
  Field.prototype = Object.create(Controllable.prototype);
  Field.prototype.constructor = Field;
  Field.prototype.get = function(context, cb){
    context.result = context.fieldData;
    cb();
  };
  Field.prototype.set = function(context, cb){
    context.result = context.fieldInput;
    cb();
  };
    ...
    const chainHandler = function(field){
      ...
      const chainCallback = function(error){
        if(error){
          ...
          result[field.id] = subcontext.result;
        }
        chain.next();
      };
      field.fire(subcontext, chainCallback);
    ...
    function chainHandler(index){
      ...
      const chainCallback = function(error){
        if(error){
          ...
          result[index] = subcontext.result;
        }
        chain.next();
      };
  ...
  Modelator.prototype.do = function(action, data, input, cb, uid, keys){
    ...
    modelator.fire(subcontext, function(error){
      if(error){
        ...
        cb(null,subcontext.result.parse());
      }
