        ...
        if(controllable.i18n){
          ...
          for(const lang of context.modelator.languages){
            ...
            for(const handler of handlers){              
              chain.add(handler, context, controllable.fieldHandlerCallback.bind(controllable, subcontext, chain));
              chain.add(controllable.handle.bind(controllable), subcontext, cb);
            }
          }
          chain.add(controller, context, controllable.fieldHandlerCallback.bind(controllable, context, chain));
          chain.add(controllable.handle.bind(controllable), context, cb);
        } else {
          for(const handler of handlers){
            chain.add(handler, context, controllable.fieldHandlerCallback.bind(controllable, context, chain));
            chain.add(controllable.handle.bind(controllable), context, cb);
          }
        }
...
const Schema = module.exports.Schema = function(config = {}){
  ...
  Schema.prototype.controller = function(context, cb){
    ...
    for(const index of indexes){
      context.errors[context.path.concat([index]).join('.')] = 'unknow-input-field'; // TODO: toDotNotation method
    }
...
const SchemaArray = module.exports.SchemaArray = function(config = {}){
  ...
  SchemaArray.prototype.controller = function(context, cb){ // cb === fieldHandlerCallback binded with context ===> function(error){ context = this}
    ...
    for(let index = 0; index < result.length; index++){
      ...
      chain.add(schemaArray.fire.bind(schemaArray, subcontext, Schema.prototype.controller));
  ...
  SchemaArray.prototype.arrayHandle = function(context, cb, next){
    ...
    if(context.result !== undefined){
      if(context.action === 'insert'){
        ...
        // check if empty (only internals _mtime _id ...)
...
const Modelator = module.exports.Modelator = function(config = {}){
  ...
  Modelator.prototype.do = function(action, data, input, cb, uid, keys){
    ...
    const context = {
      modelator, action, data, input, uid, keys, 
      fieldData : data,
      fieldInput : input,
      errors : {},
      inserts : {},
...
console.log("\n\n  MODELATOR DO - UPDATE\n".bgMagenta + "\n");
console.log(" DATA \t".black.bgWhite, JSON.stringify(resourceData2, undefined, 2), "\n");
console.log(" INPUT \t".bgCyan, JSON.stringify(updateData2, undefined, 2), "\n");
myModelator.do(
  "update",
  undefined,
  updateData2,
