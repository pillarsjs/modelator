...
const Controllable = module.exports.Controllable = function(config = {}){
  ...
  Controllable.prototype.fire = function(context, cb){
    ...
    console.log(" -> Firing: ".bgMagenta, (context.path.join(".") || '.').bold.grey, traceResults(context.result));
        ...
        if(controllable.i18n){
          ...
          for(const lang of context.modelator.languages){
            ...
            for(const handler of handlers){
              chain.add(controllable.fieldFireChain, handler, subcontext, cb);
            }
          }
          chain.add(controllable.fieldFireChain, controller, context, cb);
        } else {
          for(const handler of handlers){
            chain.add(controllable.fieldFireChain, handler, context, cb);
          }
        }
        chain.add(fieldFireEnd, context, cb).pull();
...
function fieldFireEnd(context, cb){
  // context.parent.result[context.index] = context.result;
  console.log(" <- Result: ".bgCyan, (context.path.join(".") || '.').bold.grey, traceResults(context.result));
  cb();
}

  Controllable.prototype.fieldFireChain = function(handler, context, cb, next){
    //const chain = this;
    const chainCallback = function(error){
      ...
      if(isError){
        console.log(" <- Error: ".bgRed, (context.path.join(".") || '.').bold.grey, context.errors);
...
const SchemaArray = module.exports.SchemaArray = function(config = {}){
  ...
  SchemaArray.prototype.controller = function(context, cb){
    ...
    for(let index = 0; index < context.result.length; index++){
      context.result[index] = new Tree(context.result[index]);
      context.errors[index] = new Tree(context.errors[index]);
...
const Modelator = module.exports.Modelator = function(config = {}){
  ...
  Modelator.prototype.do = function(action, data, input, cb, uid, keys){
    ...
    const fromData = ["projection", "get", "remove"].indexOf(action) >= 0; // ["insert", "update"]
    const result = fromData? fieldData : fieldInput;
    const path = [];
    const context = {modelator, action, data, input, uid, keys, owner, guest, fieldData, fieldInput, result: result, path};
