...
const Transaction = module.exports = function Transaction(context, done){
  ...
  transaction.done = done.bind(transaction);
  transaction.results = [];
  transaction.reverse = [];
  transaction.counts = [];
  transaction.gets = [];
  transaction.filters = context.filters;
  transaction.pointers = context.pointers;
  ...
  if(context.method === "count"){
    transaction.mountSentences("count", context.filters);
  } else if(context.method === "get"){
    transaction.mountSentences("get", context.filters);
  }

  transaction.chain.add(done,undefined, transaction).pull();
};
