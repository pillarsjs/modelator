...
function Driver(config){
  ...
  Driver.prototype.handle = function(transaction, pointer, sentence, collection, main, next){
    const driver = this;
    ...
    return function(error, result) {
      if(error){
        ...
        transaction.results.push({pointer,result,sentence,collection,parents});
        next();
      }
    };
  };
  Driver.prototype.insert = function(transaction, pointer, sentence, collection, main, next){
    const driver = this;
    
    // TODO: ownering check
    
    const handler = driver.handle(transaction, pointer, sentence, collection, main, next);
