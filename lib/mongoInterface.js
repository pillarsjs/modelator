...
function pointerParents(pointer){
  const parents = [];
...
function sentenceSetParents(sentence, pointer){
  for (const parent of pointer.slice(0,-1)){
    if(typeof parent !== 'string'){
      sentence['__' + parent.ref] = parent.id;
    }
  }
}
...
function Driver(config){
  ...
  Driver.prototype.sentenceRelationalControl = function(transaction, pointer, sentence, main){
    const driver = this;
    if(!main){
      for (const insert of sentence){
        sentenceSetParents(sentence, pointer);
      }
    } else {
  ...
  Driver.prototype.handle = function(method, transaction, pointer, sentence, collection, main){
    const driver = this;
    const parents = pointerParents(pointer);
    driver.sentenceRelationalControl(transaction, pointer, sentence, main);
    return function(error, result) {
      if(error){
        return transaction.done({
          method,
          pointer,
          error : method + "-main-error",
          details : error,
          sentence,
          collection,
          parents
        });
      } else if( (method === "insert" && !result.nModified) ) {
        return transaction.done({
          method,
          pointer,
          error : method + "-main-unreachable",
          details : "",
          sentence,
          collection,
          parents
        ...
        transaction.results.push({method, pointer, result, sentence, collection, parents});
        transaction.chain.next();
      }
    };
  };
  Driver.prototype.insert = function(transaction, sentence, collection, handler, main){
    const driver = this;
    // TODO: ownering check
    // sentence._id = 0; // for error.

    driver.connect(function(error, conn){
      if(!error){
        const dbc = driver.conn.collection(collection);
        const method = main? 'insert' : 'insertMany';
        dbc[method](sentence, handler);
      }
    });
  };
  Driver.prototype.update = function(transaction, sentence, collection, handler, main){
    const driver = this;
    // TODO: ownering check

    const query = {_id : sentence._id};
    const update = {
      $set : Tree.create(sentence).parse()
    };
    ...
    driver.connect(function(error, conn){
      if(!error){
        const dbc = driver.conn.collection(collection);
        const method = main? 'insert' : 'insertMany';
        dbc.update(query, sentence, handler);
      }
    });
  };
...
ModelatorMongoDriver.update = function(schema, params, cb, user, keys, transaction, main, next){
...
*/
ModelatorMongoDriver.insert = function(schema, params, cb, user, keys, transaction, main, next){
...
ModelatorMongoDriver.remove = function(schema, params, cb, user, keys, transaction, main, next){
