...
const Controllable = module.exports = function Controllable(config = {}){
  ...
  Controllable.prototype.fire = function(context, auth, next){
    ...
    if(every || context.result !== undefined){
      ...
      const keysCheck = controllable.keys.check(context.method === 'count'? 'retrieve' : context.method, context.keys);
      const keysCheckAsOwner = controllable.keys.check(context.method === 'count'? 'retrieve' : context.method, context.keys.concat(['owner']));
      const keysCheckAsGuest = controllable.keys.check(context.method === 'count'? 'retrieve' : context.method, context.keys.concat(['guest']));
  ...
  Controllable.prototype.handle = function(context, done, next){
    ...
    if(context.error){
      ...
      if(["count", "get"].indexOf(context.method)<0 && typeof context.result === 'object' && Object.keys(context.result).length === 0 ){
