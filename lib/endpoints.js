...
function modelatorAPI() {
	...
	// TO DO: Add a way to access to COUNT JAQL resource - Maybe with a new select operator for counts? {select:["user.*","!prop1.subprop2","*prop1.subprop3"],query:{...}}
		...
		modelator[exec[gw.method]](
			...
			function(error, context, transaction){
				if(error) {
					...
					gw.json(Object.assign(transaction, {errors:context.errors, params: gw.params}), {deep:5});
				}
			},
			userId, userKeyring,
