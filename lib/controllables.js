...
// TODO A: auth credentials without query original data
// TODO A: mark arrays on projection results
...
const Controllable = module.exports.Controllable = function(config = {}){
  ...
  controllable.main = config.main || false; // TODO B: review, for mark fields when is include on projection/selection as minimal retrieve
  ...
  Controllable.prototype.fire = function(context, auth, cb){
    ...
    if(every || context.result !== undefined){
      ...
      // TODO:
      // usar las projections para crear un indice de permisos facilmente consultable para la acción a realizar.
      // projection servirá como meta-información para cualquier consulta
      // en una projección exclusiva deben poder consultarse la minima projección a devolver, todos los posibles campos añadidos,
      // los que son indices y permiten utlizarlos para realizar busquedas u ordenaciones, si son indices unicos etc.
      // aparte es necesario añadir unos atributos owner y guest que marcan la necesidad de ser uno de ellos o cualquiera de los dos.
      // las projecciones se pueden pedir solas y siempre se adjuntan al resultado de cualquier otro metodo
      ...
      const keysCheck = controllable.keys.check(context.method === 'projection'? 'retrieve' : context.method, context.keys.concat(ownkeys));

      if(context.method === "projection" && context.path.length > 0){
  ...
  SchemaArray.prototype.arrayHandle = function(context, cb, next){
    ...
    if(context.result !== undefined){
      if(context.method === 'insert'){
        const resultDotPath = dotPath(context.path).slice(0,-1).join('');
        ...
        const resultDotPath = dotPath(context.path).join('');
...
const Modelator = module.exports.Modelator = function(config = {}){
  ...
  Modelator.prototype.do = function(method, entity, payload, cb, uid, keys){
    ...
    modelator.fire(context, true, function(){
      ...
      const error = Object.keys(context.errors).length > 0 || undefined;
