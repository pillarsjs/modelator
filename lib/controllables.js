...
function Transaction(context, done){
  ...
  if(context.inserts){
    for(const [pointer, sentence] of context.inserts){
      ...
      transaction.chain.add(driver.insert.bind(driver), transaction, pointer, sentence, collection, false);
    }
  }
  if(context.updates){
    for(const [pointer, sentence] of context.updates){
      const driver = pointer.slice(-1)[0].driver;
      const collection = pointer.slice(-1)[0].ref;
      transaction.chain.add(driver.update.bind(driver), transaction, pointer, sentence, collection, false);
    }
  }
  if(context.removes){
...
function traceSetencesMap(sentences, limit, space){
  ...
  for(const [pointer, sentence] of sentences){
    output[dotPath(pointer)] = limit? traceResults(sentence, limit, space) : sentence;
  }
  return traceResults(output, 100000);
}

function resultShow(error, context){

  console.log("\n\n  MODELATOR OUTPUT\n".black.bgWhite + "\n");
  console.log("\n    PROJECTION:\n".bgCyan, traceSetencesMap(context.projection), "\n");
