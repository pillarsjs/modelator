...
const Controllable = module.exports.Controllable = function(config = {}){
  ...
  Controllable.prototype.fire = function(context, auth, cb){
    ...
    const fromData = ["projection", "get", "remove"].indexOf(context.context.action) >= 0;
    const result = fromData? context.fieldData : context.fieldInput;
    context.result = result;

    debug(("  " + context.action.charAt(0).toUpperCase() + context.action.slice(1) + ": \t").bgMagenta + (" " + (context.path.join(".") || '.')).magenta, traceResults(context.fieldData));
    ...
    if(every || context.result !== undefined){
      ...
      const keysCheck = controllable.keys.check(context.action === 'projection'? 'get' : context.action, context.keys.concat(ownkeys));
  ...
  schema.schema.splice(0, 0, ...[
    new Controllable({ // really is idField TODO: id field
      ...
      on : {
        ...
        get : [function hideId(context,cb){
          let id;
...
const SchemaArray = module.exports.SchemaArray = function(config = {}){
  ...
  SchemaArray.on("get", SchemaArray.get);
};
  SchemaArray.prototype = Object.create(Controllable.prototype);
  SchemaArray.prototype.constructor = SchemaArray;
  SchemaArray.prototype.get = function(context, cb){
    const schemaArray = this;
