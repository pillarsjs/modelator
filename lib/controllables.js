...
const Controllable = module.exports.Controllable = function(config = {}){
  ...
  Controllable.prototype.handle = function(context, done, next){
    ...
    if(context.error){
      ...
      context.errors.set(dotPath(context.parents), context.error);
      done();
      ...
      if(context.parent.result[context.index] || context.result !== undefined){
        debug("  Handling:\t".bgCyan, (dotPath(context.parents)).bold.grey, traceResults(context.result));
        context.parent.result[context.index] = context.result;
      } else {
        debug("  Ignoring:\t".bgCyan, (dotPath(context.parents)).bold.grey, traceResults(context.result));
      }
...
const Schema = module.exports.Schema = function schemaController(config = {}){
  ...
  Schema.prototype.controller = function(context, done){
    ...
    for(const index of indexes){
      context.errors.set(dotPath(context.parents) + '.' + index, 'unknow-input-field');
    }
...
const SchemaArray = module.exports.SchemaArray = function(config = {}){
  ...
  SchemaArray.prototype.retrieve = function(context, done){
    const schemaArray = this;
    if(context.schemaArrayMain){ // Is the main call with complete result, no a index
  ...
  Modelator.prototype.do = function(method, entity, payload, cb, uid, keys){
    ...
    modelator.fire(context, true, function(){
      ...
      const transaction = new Transaction(context, function(error){
        if(error){
          if(error.details){
            console.log(('Transaction.' + dotPath(error.pointer)).bgRed, error.error.red, '\n Setence/Error: ' + error.details.message.grey + '\n', traceResults(error.sentence) + "\n", error.details.stack.toString().grey);
          } else {
            console.log(('Transaction.' + dotPath(error.pointer)).bgRed, error.error.red, '\n Sentence: ' + '\n', traceResults(error.sentence));
          }
        } else {
          for(const result of transaction.results){
            console.log(('Transaction.' + dotPath(result.pointer)).bgCyan, result.result.result? result.result.result : result.result, '\n' + ("(method:" + result.method + ")").yellow + ("(collection:" + result.collection + ")").yellow + ("(parents:" + result.parents + ")").yellow + traceResults(result.sentence) + "\n");
          }
        }
...
function Transaction(context, done){
  ...
  const collection = context.parents[0].ref;
  const handler = driver.handle(context.method, transaction, transaction.pointer, context.result, collection, true);

  chain.add(driver[context.method].bind(driver), transaction, context.result, collection, handler, true);

  if(context.inserts){
    for(const [pointer, sentence] of context.inserts){
      const driver = pointer.slice(-1)[0].driver;
      const collection = pointer.slice(-1)[0].ref;
      const handler = driver.handle("insert", transaction, pointer, sentence, collection, false);
      transaction.chain.add(driver.insert.bind(driver), transaction, sentence, collection, handler, false);
    }
  }
  if(context.updates){
    for(const [pointer, sentence] of context.updates){
      const driver = pointer.slice(-1)[0].driver;
      const collection = pointer.slice(-1)[0].ref;
      const handler = driver.handle("update", transaction, pointer, sentence, collection, false);
      transaction.chain.add(driver.update.bind(driver), transaction, sentence, collection, handler, false);
    }
  }
  if(context.removes){
    for(const [pointer, sentence] of context.removes){
      const driver = pointer.slice(-1)[0].driver;
      const collection = pointer.slice(-1)[0].ref;
      const handler = driver.handle("remove", transaction, pointer, sentence, collection, false);
      transaction.chain.add(driver.remove.bind(driver), transaction, sentence, collection, handler, false);
    }
  }
  ...
  const driver =   context.parents[0].driver;
  const collection = context.parents[0].ref;
  const handler = driver.handle(transaction, transaction.pointer, context.result, collection, false);

  chain.add(driver[context.method].bind(driver), transaction, context.result, collection, handler,   true);
  // chain.add(driver[context.method].bind(driver), transaction, transaction.pointer, context.result, collection, true);

  // transaction.chain.add(driver.remove.bind(driver), transaction, pointer, sentence, collection, false);


  // chain.add(driver.insert.bind(driver), transaction, sentence, collection, handler, false);

  if(context.inserts){
    for(const [pointer, sentence] of context.inserts){
      const driver = pointer.slice(-1)[0].driver;
      const collection = pointer.slice(-1)[0].ref;
      const handler = driver.handle(transaction, pointer, sentence, collection, false);
      transaction.chain.add(driver.insert.bind(driver), transaction, sentence, collection, handler, false);
    }
  }
  if(context.updates){
...
function resultShow(error, context){
  ...
  console.log("\n    ERRORS:\n".bgRed,      traceSetencesMap(context.errors), "\n");
  console.log("\n    MAIN:\n".bgCyan,       traceResults(context.result), "\n");
  console.log("\n    INSERTS:\n".bgGreen,   traceSetencesMap(context.inserts), "\n");
  console.log("\n    UPDATES:\n".bgGreen,   traceSetencesMap(context.updates), "\n");
  console.log("\n    REMOVES:\n".bgGreen,   traceSetencesMap(context.removes), "\n");
